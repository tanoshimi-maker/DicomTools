...
public static class DicomTagHelper
{
    ...
    /// <summary>
    /// Parse a backslash-separated DICOM value string into a double array.
    /// </summary>
    public static double[] GetDoubleValues(DicomDataset ds, DicomTag tag, int expectedCount)
    {
    ...
    /// <summary>
    /// Get a single integer value from a DICOM tag, with a fallback default.
    /// </summary>
    public static int GetIntValue(DicomDataset ds, DicomTag tag, int fallback = 0)
    {
        return ds.TryGetSingleValue(tag, out int val) ? val : fallback;
    }
