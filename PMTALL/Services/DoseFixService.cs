...
public class DoseFixService : IDoseFixService
{
        ...
        if (enhanceMode)
        {
            ...
            foreach (var ct in ctFiles)
            {
                if (ct.FrameOfReferenceUid != null && !ctIopCache.ContainsKey(ct.FrameOfReferenceUid))
                {
                    if (ct.Dataset != null)
                    {
                        var iop = DicomTagHelper.GetDoubleValues(ct.Dataset, DicomTag.ImageOrientationPatient, 6);
                        if (iop.Length == 6)
    ...
    private static double[] ComputeNewIpp(DicomFileInfo file, double[] newIop)
    {
        ...
        var ippOld = DicomTagHelper.GetDoubleValues(ds, DicomTag.ImagePositionPatient, 3);
        var iopOld = DicomTagHelper.GetDoubleValues(ds, DicomTag.ImageOrientationPatient, 6);
